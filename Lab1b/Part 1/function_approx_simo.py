...
val = True
val_ps = [x for x in np.arange(0.2, 0.9, 0.1)]
...
def plot_points(patterns, targets, ax):
...
def main():
    ...
    for val_p in val_ps:
        ...
        if val:
            ax.set_title(
                f'MSE:{MSE_errors[-1]:.2f} - val prop:{val_p} - MSE val:{MSE_errors_val[-1]:.2f} - hn:{HIDDEN_NODES} - lr:{LEARNING_RATE} - epochs:{EPOCHS}')
        else:
        ...
        if val:
            ...
            print(f'{MSE_errors[-1]:.2f} - val prop:{val_p} - MSE val:{MSE_errors_val[-1]:.2f}')
        # plt.show()
        # plot learning curve
